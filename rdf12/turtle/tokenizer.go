...
//
// The two packages are siblings rather than one wrapping the other. A rule is
// where two grammars may differ even when the characters it reads are the same,
// and RDF 1.2 differs in enough of them — every terminal beginning with '<',
// '>', ')', '{', '|' or '@' — that sharing the rules would mean a tokenizer
// deciding which dialect it was reading on every character. The shared parts
// are the ones that really are shared: the PN_CHARS classes and the escape
// forms all four syntaxes use, which come from internal/lex (#22).
//
...
// Token is one terminal of the Turtle grammar, with the position of its first
// character.
//
...
// TokenType is the kind of a [Token], one per terminal of the RDF 1.2 Turtle
// grammar (https://www.w3.org/TR/rdf12-turtle/#sec-grammar).
type TokenType int
...
const (
	...
	//
	// The directive is what RDF 1.2 adds to the three RDF 1.1 had. Turtle
	// spells it in both styles, where RDF 1.2 N-Triples has only the
	// SPARQL-style keyword.
	TokenVersion
...
// Tokenize reads the RDF 1.2 Turtle document in r and yields its tokens in
// order.
//
...
//
//	for token, err := range turtle.Tokenize(r) {
//		if err != nil {
//			return err
//		}
//		// ...
//	}
//
// White space between terminals is dropped, a line ending included: a Turtle
...
// Turtle allows there.
//
// Only "a", "PREFIX", "BASE" and "VERSION" may stand without a colon after
// them. Anything else that looks like a name is a prefixed name missing its
// colon, which is what this says rather than pointing at whatever followed it.
