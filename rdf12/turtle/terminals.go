...
//
// RDF 1.2 is where that character stops settling the matter. A '<' opens an
// IRIREF, a reified triple or a triple term; a ')' closes a collection or a
// triple term; a '{' is a TriG graph block in the grammar this one is the
// parent of, and an annotation block here. Each is left to the rule it hands
// off to, as the three RDF 1.1 already left there: a '[' may open a property
// list or be half of an ANON, a name may turn out to be a keyword, and a
// number may end at a '.' that belongs to the statement rather than to it.
...
// tokenizeOpenBrace decides what a '{' opened, the '{' having been consumed.
//
//	annotationBlock ::= '{|' predicateObjectList '|}'
//
// A '|' after it makes an annotation block. A '{' on its own opens a graph
// block, which belongs to TriG and not to Turtle, so it is refused here — and
// refused at the brace rather than at whatever followed it, the brace being
// the character a Turtle document may not have written.
//
// The lookahead is what the TriG delta is built on: rdf12/trig differs from
// this package by emitting an open brace here instead of reporting one, and
// that is the whole of the difference at this character. Consuming the '|'
// only once it is known to be there is what leaves that door open.
...
// without one it can only be "a", "PREFIX", "BASE" or "VERSION".
