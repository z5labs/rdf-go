...
// The rules covered here came from the RDF 1.2 Turtle tokenizer, parser and
// printer along with the rest of the files they sit in. They are exercised
// again because the copies are still separate code — internal/lex holds the
...
func TestTokenizeReaderError(t *testing.T) {
	...
	sources := []string{
		...
		"_:abc", `"o"@en`, `"o"@en-`, "ex:name", "42", "1e", "[", `"o"^`,
		"{", "GRAPH", "graph:name", "<<", "<<(", ")", ">", "|",
...
func TestParseTurtleErrors(t *testing.T) {
	sources := []string{
		...
		"<http://e/g> { <http://e/s> <http://e/p> <<( \"v\" <http://e/b> <http://e/c> )>> . }",
		"<http://e/g> { <http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> << <http://e/c> <http://e/d> <http://e/e> >> )>> . }",
		"<http://e/g> { << ( <http://e/a> ) <http://e/b> <http://e/c> >> . }",
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			err: trig.UnexpectedTokenError{
				Expected: []trig.TokenType{trig.TokenOpenBrace, trig.TokenGraph},
				Actual:   tok(2, 5, trig.TokenCloseBrace, "}"),
			},
			want: "unexpected token at line 2, column 5: CloseBrace(}), expected one of: OpenBrace, Graph",
		},
		{
			...
			want: "unexpected end of tokens at line 4, column 1, expected one of: CloseBrace",
		},
		{
			name: "unexpected name",
			err:  trig.UnexpectedNameError{Pos: pos(1, 1), Name: "grahp"},
			want: `unexpected name "grahp" at line 1, column 1`,
		},
	}
...
func TestTokenString(t *testing.T) {
	got := tok(1, 1, trig.TokenOpenBrace, "{").String()
	if want := "OpenBrace({)"; got != want {
		t.Errorf("Token.String() = %q, want %q", got, want)
	}
}

func TestPosString(t *testing.T) {
	if got, want := pos(3, 12).String(), "3:12"; got != want {
		t.Errorf("Pos.String() = %q, want %q", got, want)
	}
}

// TestDecodeTurtleTerms covers the terms TriG takes from RDF 1.2 Turtle,
// lowered inside a graph block so that the graph label rides along with each of
// them.
func TestDecodeTurtleTerms(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "a reifier written bare mints a blank node",
			src:  "<http://e/g> { << <http://e/a> <http://e/b> <http://e/c> ~ >> <http://e/p> <http://e/o> . }",
			want: "_:r <" + string(vocab.RDFReifies) + "> <<( <http://e/a> <http://e/b> <http://e/c> )>> <http://e/g> .\n" +
				"_:r <http://e/p> <http://e/o> <http://e/g> .\n",
		},
		{
			name: "a reified triple nested in a reified triple",
			src: "<http://e/g> { << << <http://e/a> <http://e/b> <http://e/c> ~ <http://e/r> >> " +
				"<http://e/d> <http://e/e> ~ <http://e/s> >> . }",
			want: "<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/a> <http://e/b> <http://e/c> )>> <http://e/g> .\n" +
				"<http://e/s> <" + string(vocab.RDFReifies) + "> <<( <http://e/r> <http://e/d> <http://e/e> )>> <http://e/g> .\n",
		},
		{
			// A block takes the reifier written immediately before it, and the
			// '~' is spent doing so.
			name: "two annotation blocks after one reifier",
			src: "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> ~ <http://e/r> " +
				"{| <http://e/q> 1 |} {| <http://e/q> 2 |} . }",
			want: "<http://e/s> <http://e/p> <http://e/o> <http://e/g> .\n" +
				"<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n" +
				`<http://e/r> <http://e/q> "1"^^<` + string(vocab.XSDInteger) + "> <http://e/g> .\n" +
				"_:b <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n" +
				`_:b <http://e/q> "2"^^<` + string(vocab.XSDInteger) + "> <http://e/g> .\n",
		},
	}
...
func TestDecodeUndefinedPrefixesEverywhere(t *testing.T) {
	sources := []string{
		...
		"<http://e/g> { <http://e/s> <http://e/p> <<( ex:a <http://e/b> <http://e/c> )>> . }",
		"<http://e/g> { << ex:a <http://e/b> <http://e/c> >> <http://e/p> <http://e/o> . }",
		"<http://e/g> { <http://e/s> <http://e/p> <http://e/o> ~ ex:r . }",
		"<http://e/g> { <http://e/s> <http://e/p> <http://e/o> {| ex:q <http://e/v> |} . }",
