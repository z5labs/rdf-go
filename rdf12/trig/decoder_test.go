...
// decodeNQuads lowers the RDF 1.2 N-Quads a test states its expectation in.
//
...
func TestDecode(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "a version directive says nothing about the dataset",
			src: "VERSION \"1.2\"\n" +
		...
		{
			// A triple term is a term wherever it stands, so it lowers the same
			// inside a block as outside one and asserts nothing of its own.
			name: "a triple term inside a block",
			src:  "<http://e/g> { <http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> <http://e/c> )>> . }",
			want: "<http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> <http://e/c> )>> <http://e/g> .",
		},
		{
			name: "a nested triple term inside a block",
			src: "<http://e/g> { <http://e/s> <http://e/p> " +
				"<<( <http://e/a> <http://e/b> <<( <http://e/c> <http://e/d> <http://e/e> )>> )>> . }",
			want: "<http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> " +
				"<<( <http://e/c> <http://e/d> <http://e/e> )>> )>> <http://e/g> .",
		},
		{
			// The rdf:reifies triple the sugar stands for belongs to the block's
			// graph, and the enclosed statement is not asserted at all.
			name: "a reified triple inside a block",
			src:  "<http://e/g> { << <http://e/a> <http://e/b> <http://e/c> ~ <http://e/r> >> <http://e/p> <http://e/o> . }",
			want: "<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/a> <http://e/b> <http://e/c> )>> <http://e/g> .\n" +
				"<http://e/r> <http://e/p> <http://e/o> <http://e/g> .\n",
		},
		{
			name: "an annotation inside a block asserts the triple it describes",
			src:  "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> ~ <http://e/r> {| <http://e/q> <http://e/v> |} . }",
			want: "<http://e/s> <http://e/p> <http://e/o> <http://e/g> .\n" +
				"<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n" +
				"<http://e/r> <http://e/q> <http://e/v> <http://e/g> .\n",
		},
		{
			name: "an annotation with no reifier mints one in the block's graph",
			src:  "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> {| <http://e/q> <http://e/v> |} . }",
			want: "<http://e/s> <http://e/p> <http://e/o> <http://e/g> .\n" +
				"_:r <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n" +
				"_:r <http://e/q> <http://e/v> <http://e/g> .\n",
		},
		{
			// The same triple annotated in two graphs is two reifications, each
			// standing where it was written.
			name: "one triple annotated in two graphs",
			src: "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> ~ <http://e/r> . }\n" +
				"<http://e/h> { <http://e/s> <http://e/p> <http://e/o> ~ <http://e/r> . }\n",
			want: "<http://e/s> <http://e/p> <http://e/o> <http://e/g> .\n" +
				"<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n" +
				"<http://e/s> <http://e/p> <http://e/o> <http://e/h> .\n" +
				"<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/h> .\n",
		},
		{
			name: "a directional literal inside a block",
			src:  `<http://e/g> { <http://e/s> <http://e/p> "v"@en--ltr, "w"@ar--rtl, "x"@en . }`,
			want: `<http://e/s> <http://e/p> "v"@en--ltr <http://e/g> .` + "\n" +
				`<http://e/s> <http://e/p> "w"@ar--rtl <http://e/g> .` + "\n" +
				`<http://e/s> <http://e/p> "x"@en <http://e/g> .` + "\n",
		},
		{
...
// quads of a nested collection come before the quad that uses it, and every one
// of them carries the label of the block it was written in.
...
// TestDecodeAnnotatesAfterAsserting covers the one construct that comes after
// rather than before, read inside a block so that the graph is checked too.
func TestDecodeAnnotatesAfterAsserting(t *testing.T) {
	quads := decodeTrig(t, "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> ~ <http://e/r> . }")

	if len(quads) != 2 {
		t.Fatalf("Decode() yielded %d quads, want 2:\n%s", len(quads), formatQuads(quads))
	}
	if quads[0].Predicate != "http://e/p" {
		t.Errorf("the annotated quad came second:\n%s", formatQuads(quads))
	}
	if quads[1].Predicate != vocab.RDFReifies {
		t.Errorf("the rdf:reifies quad came first:\n%s", formatQuads(quads))
	}
	for i, quad := range quads {
		if !rdf.IRI("http://e/g").Equal(quad.Graph) {
			t.Errorf("quad %d is in graph %v, want <http://e/g>", i, quad.Graph)
...
func TestDecodeErrors(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "a base direction the data model refuses",
			src:  `<http://e/g> { <http://e/s> <http://e/p> "v"@en--up . }`,
