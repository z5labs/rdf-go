...
func TestPrint(t *testing.T) {
	testCases := []struct {
		...
		{
			// The annotation's braces are not the block's, and neither one is
			// mistaken for the other on the way out.
			name: "an annotation inside a block",
		...
		{
			name: "a reified triple standing alone inside a block",
			src:  "@prefix ex: <http://e/> .\nex:g { << ex:a ex:b ex:c ~ ex:r >> }",
			want: "@prefix ex: <http://e/> .\n" +
				"ex:g {\n" +
				"    << ex:a ex:b ex:c ~ ex:r >> .\n" +
				"}\n",
		...
		{
			name: "a version directive keeps its place among the statements",
			src:  "VERSION \"1.2\"\n<http://e/g> { }",
			want: "VERSION \"1.2\"\n<http://e/g> {\n}\n",
		},
		{
...
// TestPrintWrapsInsideBlocks covers the layout a block adds: its triples begin
// one level in, and what they break wraps from there rather than from the
// margin.
func TestPrintWrapsInsideBlocks(t *testing.T) {
	src := "@prefix ex: <http://e/> .\n" +
		"ex:g { ex:subject ex:predicate ex:objectOne, ex:objectTwo, ex:objectThree . }"
	...
	if got := printToString(t, parse(t, src), trig.WithLineWidth(40)); got != want {
...
func TestPrintRoundTrip(t *testing.T) {
	sources := []string{
		...
		"@prefix ex: <http://e/> .\nex:g { << ex:a ex:b ex:c >> ex:p ex:o . }",
		"@prefix ex: <http://e/> .\nex:g { << ex:a ex:b ex:c ~ >> ex:p ex:o . }",
		"@prefix ex: <http://e/> .\nex:g { << ex:a ex:b ex:c ~ ex:r >> . }",
		"@prefix ex: <http://e/> .\nex:g { << << ex:a ex:b ex:c >> ex:d ex:e >> ex:p ex:o . }",
		"@prefix ex: <http://e/> .\nex:g { ex:s ex:p ex:o ~ ex:r . }",
		"@prefix ex: <http://e/> .\nex:g { ex:s ex:p ex:o {| ex:q ex:v |} . }",
		"@prefix ex: <http://e/> .\nex:g { ex:s ex:p ex:o ~ ex:r {| ex:q ex:v |} {| ex:x ex:y |} . }",
...
func TestPrintWriterError(t *testing.T) {
	...
	doc := parse(t, "@prefix ex: <http://e/> .\nex:g { ex:s a ex:C ; ex:p ( 1 2 ) , [ ex:q 1 ] {| ex:r 2 |} . }\n# note")
...
func TestEncode(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// The two terms the RDF 1.1 encoder has to refuse, both written
			// here — inside a block, which changes nothing about either.
			name:   "a triple term object needs no refusal",
			nquads: "<http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> <http://e/c> )>> <http://e/g> .",
			want:   "@prefix ex: <http://e/> .\nex:g {\n    ex:s ex:p <<( ex:a ex:b ex:c )>> .\n}\n",
		},
		{
			name:   "a literal carrying a base direction needs no refusal",
			nquads: `<http://e/s> <http://e/p> "v"@en--ltr <http://e/g> .`,
			want:   "@prefix ex: <http://e/> .\nex:g {\n    ex:s ex:p \"v\"@en--ltr .\n}\n",
		},
		{
			// The sugar is Print's to write: a reification arrives as the
			// ordinary triple it is, and is written as one.
			name: "a reification is written as the rdf:reifies triple it is",
			nquads: "<http://e/s> <http://e/p> <http://e/o> <http://e/g> .\n" +
				"<http://e/r> <" + string(vocab.RDFReifies) + "> <<( <http://e/s> <http://e/p> <http://e/o> )>> <http://e/g> .\n",
			want: "@prefix ex: <http://e/> .\n" +
				"ex:g {\n" +
				"    ex:s ex:p ex:o .\n" +
				"    ex:r <" + string(vocab.RDFReifies) + "> <<( ex:s ex:p ex:o )>> .\n" +
				"}\n",
		},
	}
...
// in either graph, inside a triple term, and as a graph's name.
func TestEncodeRelabelsAcrossGraphs(t *testing.T) {
	...
	quads := []rdf.Quad{
		...
		{Triple: rdf.Triple{
			Subject:   node,
			Predicate: "http://e/p",
			Object:    rdf.TripleTerm{Subject: node, Predicate: "http://e/q", Object: node},
		}},
	}
	...
	want := "_:b0 <http://e/p> <<( _:b0 <http://e/q> _:b0 )>> .\n" +
		"_:b0 {\n    _:b0 <http://e/p> _:b0 .\n}\n"
	if got := encodeToString(t, d); got != want {
		t.Errorf("Encode() = %q, want %q", got, want)
	}
...
func TestPrintSuppliesMissingKeywords(t *testing.T) {
	doc := &trig.Document{
		Statements: []trig.Statement{
			...
			&trig.VersionDirective{Version: "1.2"},
...
func zeroAnnotation(item trig.Annotation) {
	switch n := item.(type) {
	case *trig.Reifier:
		n.Pos = trig.Pos{}
		if n.ID != nil {
...
func zeroTerm(node trig.Term) {
	...
	case *trig.BlankNodePropertyList:
		n.Pos = trig.Pos{}
		zeroPredicates(n.Predicates)
	case *trig.TripleTerm:
		n.Pos = trig.Pos{}
		zeroTerm(n.Subject)
		zeroTerm(n.Verb)
		zeroTerm(n.Object)
	case *trig.ReifiedTriple:
		n.Pos = trig.Pos{}
		zeroTerm(n.Subject)
		zeroTerm(n.Verb)
		zeroTerm(n.Object)
