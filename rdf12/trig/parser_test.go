...
func TestParseGraphBlocks(t *testing.T) {
	testCases := []struct {
		...
		{
			// The other form the grammar lets stand alone, and the one RDF 1.2
			// adds: a reified triple has said its rdf:reifies by the time the
			// angles close.
			name:        "a reified triple needs no predicates of its own",
...
// TestParseBracesAreNeverConfused is the parser's half of the '{' versus "{|"
// question: whatever stands before a brace, an annotation stays an annotation
// and a graph block stays a graph block.
func TestParseBracesAreNeverConfused(t *testing.T) {
	testCases := []struct {
		name string
		src  string
		want []string
		...
		{
			name: "an annotated triple followed by a graph block",
			src: "@prefix ex: <http://e/> .\n" +
				"ex:s ex:p ex:o {| ex:q ex:v |} .\n" +
				"ex:g { ex:s ex:p ex:o . }\n",
			want: []string{"*trig.PrefixDirective", "*trig.Triples", "*trig.GraphBlock"},
		},
		{
			name: "an annotated triple ending a graph block",
			src: "@prefix ex: <http://e/> .\n" +
				"ex:g { ex:s ex:p ex:o {| ex:q ex:v |} }\n",
			want: []string{"*trig.PrefixDirective", "*trig.GraphBlock"},
		},
		{
			name: "an annotation and a block written without white space",
			src:  "@prefix ex: <http://e/> .\nex:s ex:p ex:o{|ex:q ex:v|}.\nex:g{ex:s ex:p ex:o}\n",
			want: []string{"*trig.PrefixDirective", "*trig.Triples", "*trig.GraphBlock"},
		},
		{
			name: "a label whose block holds an annotation on every object",
			src: "@prefix ex: <http://e/> .\n" +
				"ex:g { ex:s ex:p ex:a {| ex:q 1 |}, ex:b {| ex:q 2 |} . }\n",
			want: []string{"*trig.PrefixDirective", "*trig.GraphBlock"},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(doc.Statements) != len(tc.want) {
				t.Fatalf("parsed %d statements, want %d", len(doc.Statements), len(tc.want))
			}
			for i, statement := range doc.Statements {
				if got := reflect.TypeOf(statement).String(); got != tc.want[i] {
					t.Errorf("statement %d is a %s, want a %s", i, got, tc.want[i])
				}
			}
		})
	}
...
// TestParseAnnotationInsideBlock pins the shape of an annotated triple read
// inside a block: the block holds the triples, and the annotation belongs to
// the object it was written after.
func TestParseAnnotationInsideBlock(t *testing.T) {
	doc := parse(t, "@prefix ex: <http://e/> .\nex:g { ex:s ex:p ex:o ~ ex:r {| ex:q ex:v |} . }\n")

	b, ok := doc.Statements[1].(*trig.GraphBlock)
	if !ok {
		t.Fatalf("parsed a %T, want a *trig.GraphBlock", doc.Statements[1])
	}
	if len(b.Triples) != 1 {
		t.Fatalf("the block held %d sets of triples, want 1", len(b.Triples))
	}

	objects := b.Triples[0].Predicates[0].Objects
	if len(objects) != 1 {
		t.Fatalf("the verb had %d objects, want 1", len(objects))
	}

	annotation := objects[0].Annotation
	if len(annotation) != 2 {
		t.Fatalf("the object carried %d annotation items, want 2", len(annotation))
	}
	if _, ok := annotation[0].(*trig.Reifier); !ok {
		t.Errorf("item 0 is a %T, want a *trig.Reifier", annotation[0])
	}
	if _, ok := annotation[1].(*trig.AnnotationBlock); !ok {
		t.Errorf("item 1 is a %T, want a *trig.AnnotationBlock", annotation[1])
	}
}
...
func TestParseDefaultGraphStatements(t *testing.T) {
	testCases := []struct {
		...
		{name: "a reified triple subject", src: "<< <http://e/s> <http://e/p> <http://e/o> >> ."},
		{name: "a reified triple subject with more to say", src: "<< <http://e/s> <http://e/p> <http://e/o> >> <http://e/q> <http://e/r> ."},
		{name: "a triple term object", src: "<http://e/s> <http://e/p> <<( <http://e/a> <http://e/b> <http://e/c> )>> ."},
	}
...
func TestParseMixedDocument(t *testing.T) {
	src := "@prefix ex: <http://e/> .\n" +
		...
		"{ ex:s ex:q ex:r . }\n" +
		"ex:g { ex:s ex:p ex:o . }\n" +
		"GRAPH ex:h { ex:s ex:p <<( ex:a ex:b ex:c )>> . }\n" +
		"@version \"1.2\" .\n" +
		"PREFIX f: <http://f/>\n" +
		"f:s f:p f:o .\n"
	...
	want := []string{
		"*trig.PrefixDirective", "*trig.VersionDirective", "*trig.Triples",
		"*trig.GraphBlock", "*trig.GraphBlock", "*trig.GraphBlock",
		"*trig.VersionDirective", "*trig.PrefixDirective", "*trig.Triples",
	}
...
// subjectTokens is what a block or a statement may begin with, which is the
// list several errors below name.
var subjectTokens = []trig.TokenType{
	trig.TokenIRIRef, trig.TokenPNameNS, trig.TokenPNameLN,
	trig.TokenBlankNodeLabel, trig.TokenAnon,
	trig.TokenOpenParen, trig.TokenOpenBracket, trig.TokenReifiedTripleOpen,
}

var verbTokens = []trig.TokenType{
	trig.TokenIRIRef, trig.TokenPNameNS, trig.TokenPNameLN, trig.TokenA,
}

var labelTokens = []trig.TokenType{
	trig.TokenIRIRef, trig.TokenPNameNS, trig.TokenPNameLN,
	trig.TokenBlankNodeLabel, trig.TokenAnon,
}
	...
	testCases := []struct {
	...
	}{
		{
			...
			want: trig.UnexpectedEndOfTokensError{
				Expected: append(append([]trig.TokenType{}, subjectTokens...), trig.TokenCloseBrace),
				Pos:      pos(1, 55),
			},
		},
		{
		...
		{
			// The one RDF 1.2 adds to the list: a reified triple is a term the
			// subject position admits and labelOrSubject does not.
			name: "a reified triple cannot label a graph",
			src:  "<< <http://e/s> <http://e/p> <http://e/o> >> { }",
			want: trig.UnexpectedTokenError{
				Expected: verbTokens,
				Actual:   tok(1, 46, trig.TokenOpenBrace, "{"),
			},
		},
		{
		...
		{
			// A directive belongs to the document, and a block holds triples.
			name: "a directive may not stand inside a block",
			src:  "<http://e/g> { @prefix ex: <http://e/> . }",
			want: trig.UnexpectedTokenError{
				Expected: subjectTokens,
				Actual:   tok(1, 16, trig.TokenPrefix, "@prefix"),
			},
		},
		{
			name: "an annotation block that is never closed",
			src:  "<http://e/g> { <http://e/s> <http://e/p> <http://e/o> {| <http://e/q> <http://e/v> }",
			want: trig.UnexpectedTokenError{
				Expected: []trig.TokenType{trig.TokenAnnotationClose},
				Actual:   tok(1, 84, trig.TokenCloseBrace, "}"),
			},
		},
	}
