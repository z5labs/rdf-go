// This file began as a copy of rdf12/turtle/printer.go and keeps its shape: the
...
//
// What TriG adds is the graph block, which is a statement that holds statements:
// [printer.triples] gives up the '.' that ends it, so that a block can write one
// after each of its own and leave the last off where the grammar allows, and
// [encoder.document] builds a block per named graph rather than one flat list.
//
// The two errors the RDF 1.1 printer has for a term it cannot spell —
// ErrBaseDirection and ErrTripleTerm — are absent here, RDF 1.2 TriG having a
// syntax for both.
...
// Print writes doc to w as RDF 1.2 TriG.
//
...
// inside a statement or inside a graph block follows it rather than
...
// Encode writes dataset to w as RDF 1.2 TriG.
//
...
//
// Everything the RDF 1.2 data model can hold has a TriG spelling here, so
...
//	:r rdf:reifies <<( :s :p :o )>> .
//
// The two sugars are never produced. The same dataset could be written
...
// reified triple is asserted elsewhere in the same graph, whether the reifier
// is the subject of anything else, and whether the triple term is referred to
// more than once — three questions about the whole of a graph, asked of a
// function that groups triples by subject and otherwise writes them as they
// arrive. Getting any of them wrong writes a different dataset, since the sugar
// states triples of its own: "<< :s :p :o ~ :r >>" and ":s :p :o ~ :r" differ in
// exactly the triple ":s :p :o".
//
...
// who wants a faithful dataset and does not mind how it reads uses Encode.
//
// # Streaming
//
// Unlike [Decode] this does not stream, a dataset being in memory already. A
// caller encoding more than fits in memory wants N-Quads, which needs no
// grouping.
//
// It reports [ErrNilDataset] for a nil dataset, [ErrInvalidPrefix] for a prefix
// binding that cannot be written, the error from [rdf.Quad.Validate] for a quad
// that is not one — which is what reports a triple term standing as a subject,
// however deeply nested — and otherwise the first error from w.
//
...
// term RDF 1.2 admits has an RDF 1.2 TriG spelling. The one term that cannot
