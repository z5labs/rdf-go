// This file began as a copy of rdf12/turtle/ast.go and keeps its shape: the
// same Document, the same Statement and Term interfaces, and the same nodes for
// everything RDF 1.2 Turtle can write — [TripleTerm], [ReifiedTriple],
// [Reifier] and [AnnotationBlock] included.
//
// What TriG adds is [GraphBlock], the one node with no Turtle equivalent, and
// with it the fourth position: a statement written inside a block is said of
// the graph the block names.
//
...
// Document is a parsed TriG document.
//
...
type Document struct {
	...
	// Statements are the document's directives, triples and graph blocks, in
	// the order they were written. A directive applies to what follows it, so
	// the order is part of what the document means and not merely how it was
	// laid out.
...
// [*BaseDirective], a [*VersionDirective], a [*Triples] or a [*GraphBlock].
//
//	trigDoc   ::= (directive | block)*
...
//
// A [*Triples] standing here is a statement written outside any block, which
// belongs to the dataset's default graph (TriG §2.1).
...
//
// It stands both as a statement of the document, said of the default graph,
// and as one statement of a [*GraphBlock], said of the graph that block names.
// The two read alike but for the terminator, which the grammar lets a block
// leave off the last of its statements, and which this node does not record.
//
...
// GraphBlock is a graph written out in braces.
//
...
//
// This is the whole of what TriG adds to Turtle. The three ways of writing a
// block are all one node, told apart by two fields: Label is nil when the
// block named no graph, and Keyword is empty when it was written without the
// "GRAPH" that may precede a label. Both are kept as written so a printer can
// put back the form the author chose, the three meaning the same thing and not
// reading the same.
//
// The '{' that opens a block is not the "{|" that opens an [*AnnotationBlock],
// and the two nodes are as far apart as the terminals are: an annotation
// describes the triple it was written after, and a graph block says which
// graph its triples belong to.
type GraphBlock struct {
	Pos Pos

	// Keyword is the "GRAPH" keyword as written, or "" if the block had none.
	// The keyword is case-insensitive, so the spelling is worth keeping.
	Keyword string

	// Label is the graph's name: an [*IRIRef], a [*PrefixedName], a
	// [*BlankNode] or an [*Anon]. It is nil for a block that named no graph,
	// whose triples belong to the default graph (TriG §2.2).
	Label Term

	// Triples are the block's statements, in the order they were written. It
	// is empty for an empty block, which names a graph and says nothing about
	// it.
	Triples []*Triples
}

// Position returns the position of the "GRAPH" keyword, of the label, or of
// the '{' — whichever the block began with.
func (b *GraphBlock) Position() Pos { return b.Pos }

func (*GraphBlock) statement() {}
...
//
// Whether it is absolute is not decided here: a TriG document may write a
