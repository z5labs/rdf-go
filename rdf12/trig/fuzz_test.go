...
// graph blocks hold the whole of Turtle, whose collections, blank node
// property lists, triple terms and annotations all nest, so a mutation that
// adds a brace or a bracket adds a level of recursion, and the parser has to
// end rather than descend for as long as they keep coming.
//
...
//
// The seed corpus is both TriG suites, whose bad-syntax documents are the
// negative cases. An RDF 1.2 parser reads RDF 1.1, so the older suite is as
// much this parser's input as the newer one.
func FuzzParse(f *testing.F) {
	seeds, err := fuzzcorpus.Seeds(".trig", "rdf12/rdf-trig", "rdf11/rdf-trig")
	if err != nil {
