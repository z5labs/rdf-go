// This file is a verbatim copy of rdf12/ntriples/tokenizer.go, differing only
// in the package name and in the specification it names.
//
...
// terminals, so the two tokenizers have nothing to disagree about. What RDF
// 1.2 adds over RDF 1.1 — the "<<(" and ")>>" of a triple term, the base
// direction of a LANG_DIR, and the VERSION keyword — it adds to both syntaxes
// alike.
//
// The character classes and the escape forms that all four syntaxes share live
// in internal/lex (#22). What is left here is duplicated deliberately rather
// than overlooked: Pos, Token, TokenType and the error types are this package's
// own API, which an internal package cannot hold, and the tokenizer struct, the
// combinators and the rules built on them are what a grammar is free to differ
// in. Any fix to one copy still belongs in the other.
...
// Token is one terminal of the N-Quads grammar, with the position of its
// first character.
//
...
// TokenType is the kind of a [Token], one per terminal of the N-Quads
// grammar (https://www.w3.org/TR/rdf12-n-quads/#sec-grammar).
type TokenType int
...
const (
	...
	// TokenTripleTermClose is the ")>>" that closes a triple term.
	TokenTripleTermClose

	// TokenVersion is the VERSION keyword introducing a version directive. The
	// keyword is case-sensitive — RDF 1.2 N-Quads differs from Turtle here —
	// so its value is always "VERSION".
	TokenVersion
...
// a hyphen, a UCHAR short of its digits, a "<<(" short of its parenthesis.
//
...
// Tokenize reads the N-Quads document in r and yields its tokens in order.
//
...
//
//	for token, err := range nquads.Tokenize(r) {
//		if err != nil {
//			return err
//		}
//		// ...
...
// not there. It is what reads the tail of a delimiter whose first character
// has already identified it: the "ERSION" of a VERSION keyword, the ">>" of a
// triple term's closing bracket.
//
...
//
// Every terminal but one is identified by its first character alone. The
// exception is '<', which begins both an IRIREF and the "<<(" of a triple
// term, and which [tokenizeAngle] settles with a character of lookahead.
func tokenizeDocument(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
	return skipSpace(func(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
		...
		case r == ')':
			return tokenizeTripleTermClose(pos)
		case r == 'V':
			return tokenizeVersion(pos)
		case r == '_':
			return tokenizeBlankNodeLabel(pos)
		case r == '"':
			return tokenizeStringLiteral(pos)
		case r == '@':
			return tokenizeLangDir(pos)
		case r == '^':
...
// tokenizeAngle settles which of the two terminals beginning with '<' this is,
// the '<' having been consumed:
//
//	IRIREF     ::= '<' ([^#x00-#x20<>"{}|^`\] | UCHAR)* '>'
//	tripleTerm ::= '<<(' subject predicate object ')>>'
//
// One character of lookahead is enough, and it is only ever lookahead: a
// second '<' is consumed, since an IRIREF excludes it and so no other terminal
// can want it, and anything else is put back unread for [tokenizeIRIRef] to
// read as the first character of the IRI.
//
// A '<' that is not followed by "<(" is therefore reported where it stands,
// exactly as RDF 1.1 reports it — "<<" opens nothing in either version of the
// grammar.
func tokenizeAngle(pos Pos) tokenizerAction {
	return func(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
		...
		if err != nil {
			if errors.Is(err, io.EOF) {
				// A '<' at the very end of the input opens an IRIREF that
				// never closes, which is what tokenizeIRIRef reports.
		...
		if err := t.expect("("); err != nil {
			...
			if errors.As(err, &unexpected) {
				// The parenthesis is missing, so the second '<' was never the
				// opener it looked like and is the character that cannot be
				// here.
				return yieldErrorOr(UnexpectedCharacterError{Pos: before.pos, R: '<'}, nil)
			}
			return yieldErrorOr(err, nil)
		}
...
// tokenizeTripleTermClose reads the ">>" of the ")>>" that closes a triple
// term, the ')' having been consumed.
func tokenizeTripleTermClose(pos Pos) tokenizerAction {
	return func(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
		if err := t.expect(">>"); err != nil {
			return yieldErrorOr(err, nil)
		}
...
// tokenizeVersion reads the rest of the VERSION keyword, the 'V' having been
// consumed:
//
//	versionDirective ::= 'VERSION' versionSpecifier
//	versionSpecifier ::= STRING_LITERAL_QUOTE
//
// The keyword is quoted in the grammar and so is case-sensitive: N-Quads
// differs from Turtle here, where directives may be written in either case. A
// lowercase "version" begins no terminal at all and is refused by
// [tokenizeDocument] before it reaches this rule.
//
// The specifier that follows is an ordinary string literal, tokenized as one.
func tokenizeVersion(pos Pos) tokenizerAction {
	return func(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
		if err := t.expect("ERSION"); err != nil {
			return yieldErrorOr(err, nil)
		}

		return yieldTokenThen(
			Token{Pos: pos, Type: TokenVersion, Value: []byte("VERSION")},
			tokenizeDocument,
		)
	}
}
...
// tokenizeLangDir reads the rest of a LANG_DIR, the '@' having been consumed:
//
...
func tokenizeLangDir(pos Pos) tokenizerAction {
	return func(t *tokenizer, yield func(Token, error) bool) tokenizerAction {
