...
// Document is a parsed N-Quads document.
//
...
type Document struct {
	...
	// written: a [*Quad] for each quad and a [*VersionDirective] for each
	// version announcement.
	//
	// RDF 1.1 N-Quads had only quads to record, and the rdf11 package keeps
	// them in a list of its own. Here the two kinds share a list because the
	// order they were written in is what a version directive means: it
	// announces the version of the part of the document that follows it, so a
	// directive that has lost its place among the quads has lost what it said.
...
// Statement is one statement of a document: a [*Quad] or a
// [*VersionDirective].
//
...
// wherever a quad may stand, and RDF 1.2 Turtle §2.4 — which N-Quads refers to
// by way of N-Triples for the rules they share — says a document may carry
// more than one, each applying to the part of the document that follows it. So
// the parser records every directive where it stands and refuses none for its
// placement.
//
...
// Quad is one quad statement of a document.
//
...
// [*TripleTerm] is a [SubjectTerm] or a [GraphTerm], so neither can be a
// subject or a graph label.
type Quad struct {
	...
	// Graph is the label of the graph the statement belongs to: an [*IRIRef]
	// or a [*BlankNode], or nil where the statement was written without one
	// and so belongs to the dataset's default graph.
	//
	// This is the whole of what N-Quads adds to N-Triples. A triple term is
	// not among the alternatives: RDF 1.2 admits one in the object position
	// alone, and graphLabel is the RDF 1.1 production unchanged.
	Graph GraphTerm
}

// Position returns the position of the subject, where the statement begins.
func (q *Quad) Position() Pos { return q.Pos }
...
// well contain one, but no caller assembling a [Quad] or a [TripleTerm] by
// hand can express it.
...
// GraphTerm is a term that may label the graph a statement belongs to.
//
//	graphLabel ::= IRIREF | BLANK_NODE_LABEL
//
// The grammar gives graphLabel the same alternatives it gives subject, so this
// is the same interface under the name that reads correctly where a graph is
// meant. Both positions identify something rather than describing it, which is
// why neither a literal nor a triple term stands in either: RDF 1.2 widened
// the object production alone.
type GraphTerm = SubjectTerm
...
// arbitrary one: the subject production a triple term shares with a quad
// admits an IRIREF and a blank node label and nothing else, and graphLabel,
// which N-Quads adds, admits exactly the same two.
//
...
// not allow it: a literal as a subject, a triple term as a subject or a graph
// label, a blank node as a predicate, or a term where the '.' ending a
// statement was due.
...
// Parse reads the N-Quads document in r and returns its syntax tree.
//
...
// [Document]; [Decode] lowers each quad and hands it on the moment its '.'
...
type sink interface {
	// quad receives a parsed quad. Returning an error stops parsing, and the
	// error is what parse reports.
	quad(*Quad) error

	// version receives a parsed version directive.
	//
	// It reports no error, unlike quad: a directive announces a version rather
	// than saying anything, so there is nothing here for a sink to refuse and
	// nothing to hand a consumer that could ask it to stop.
...
// quad appends the quad to the document.
func (d *Document) quad(q *Quad) error {
	d.Statements = append(d.Statements, q)
	return nil
}

// version appends the directive to the document, in its place among the quads
// it announces the version of.
...
// parseQuad reads one quad.
//
//	quad ::= subject predicate object graphLabel? '.'
//
// Nothing may come between the parts, not even a comment: a comment runs to the
// end of its line, and a statement does not outlive its own line.
func parseQuad(p *parser, s sink) (parserAction[sink], error) {
...
// parseGraphLabel reads the graph label a statement may carry, and reports nil
// where it carries none.
//
//	graphLabel ::= IRIREF | BLANK_NODE_LABEL
//
// The label is optional, so what follows the object is either one or the '.'
// that ends the statement. Deciding between them takes a look at the next
// token and no more, the three being distinct terminals.
//
// RDF 1.2 leaves this production alone: a triple term stands in the object
// position and nowhere else, so a "<<(" here is not a label but the token that
// makes the '.' the parser then requires missing.
//
// A statement with no label belongs to the dataset's default graph. That is
// the absence of a label meaning something, not a label that happens to be
// missing, which is why nil is a legitimate value here rather than a gap.
func parseGraphLabel(p *parser) (GraphTerm, error) {
	tok, err, ok := p.peek()
	if err != nil {
		return nil, err
	}
	if !ok {
		// The input ended after the object. Let expect(TokenDot) report it,
		// so that a truncated statement is described by what is missing from
		// it rather than by the optional part that was never required.
		return nil, nil
	}
...
// parseSubject reads the subject of a quad or of a triple term.
//
//	subject ::= IRIREF | BLANK_NODE_LABEL
//
...
// parsePredicate reads the predicate of a quad or of a triple term.
//
//	predicate ::= IRIREF
//
...
// parseObject reads the object of a quad or of a triple term.
//
