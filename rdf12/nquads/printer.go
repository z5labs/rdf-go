...
// Print writes doc to w as RDF 1.2 N-Quads.
//
...
//
// The writing itself is canonical: one space between the parts of a statement,
// the graph label included, and after the "<<(" of a triple term, one statement
// to a line, and the escaping of RDF 1.2 N-Triples §3 within a quoted string.
// See [Encode] for where that canonical form comes from, N-Quads defining none
// of its own.
//
...
// printQuad writes one quad statement.
//
//	quad ::= subject predicate object graphLabel? '.'
//
// The single space between the parts is what canonical form requires, and there
// is no reason to write anything else. A statement with no label is written
// without one, which is how the default graph is spelled.
func printQuad(pr *printer, q *Quad) {
	pr.write(term(q.Subject))
	pr.write(" ")
	pr.write(term(q.Predicate))
	pr.write(" ")
	pr.write(term(q.Object))
	if q.Graph != nil {
		pr.write(" ")
		pr.write(term(q.Graph))
	}
	pr.write(" .")
}
...
// Encode writes quads to w as canonical RDF 1.2 N-Quads.
//
// # Where the canonical form comes from
//
// The N-Quads specification defines no canonical form of its own. RDF 1.2
// N-Triples §3 does, and N-Quads differs from N-Triples by one optional graph
// label, so the rules carried over here are §3's with the label written after
// the object and before the '.', separated by the same single space as
// everything else. That is the form RDF Dataset Canonicalization means when it
// speaks of canonical N-Quads.
//
// Canonical form is what makes output comparable: the same dataset written by
...
// escapes exactly the characters §3 says it must — seven take an ECHAR where
// RDF 1.1 escaped four — a language tag is written in lowercase, and every
// statement ends with a line feed, the last one included.
//
// There is no VERSION directive — §3 forbids one — and there are no comments,
// canonical N-Quads having none. A caller wanting to keep either has a
// [Document] and wants [Print].
//
// A triple term object is written as "<<( s p o )>>", nested as deeply as the
// term is. Everything the RDF 1.2 data model can hold has an N-Quads spelling
// here, so unlike the RDF 1.1 encoder this one refuses no term for want of
// syntax.
//
// # What canonical form does and does not fix
//
// §3 settles the writing — the spacing, the escaping, the case of a language
// tag, the absence of comments and of a version directive — so a ground dataset
...
// the process, so a dataset with blank nodes read and written again is the same
// dataset and need not be the same bytes. Producing labels that are themselves
// canonical is a separate problem, and a much larger one.
//
// Encoding stops at the first error, which is either the one from w or the one
// from [rdf.Quad.Validate] if a quad could not be written as N-Quads at all. A
// literal subject cannot, and neither can a triple term subject — RDF 1.2
// admits a triple term in the object position alone — so both are reported as
// [rdf.ErrInvalidSubject] rather than written in a spelling no parser would
// read back. The check recurses, so a triple term standing as the subject of
// another triple term is reported however deep it is. A graph label that is
// neither an IRI nor a blank node is reported the same way, as
// [rdf.ErrInvalidGraphName]: graphLabel is the RDF 1.1 production unchanged, so
// a triple term has no spelling there either.
//
...
func Encode(w io.Writer, quads iter.Seq[rdf.Quad]) error {
	...
	for quad := range quads {
		...
		b.Reset()
		writeTerm(&b, quad.Subject)
		b.WriteByte(' ')
		b.WriteString(quad.Predicate.String())
		b.WriteByte(' ')
		writeTerm(&b, quad.Object)
		if quad.Graph != nil {
			b.WriteByte(' ')
			writeTerm(&b, quad.Graph)
		}
		b.WriteString(" .\n")
...
//
// It is reached only for a term of a quad that [rdf.Quad.Validate] accepted, so
// the four cases are exhaustive and none of them is nil. A nil graph label is
// the default graph and is never written at all, which is what leaving it out
// means.
