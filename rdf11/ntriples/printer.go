...
// one statement to a line, and the escaping of §4 — so a document that carries
// no comments prints in canonical form. A document that does carry them cannot
// be canonical, canonical N-Triples having none.
//
...
func printNext(statement, comment int) printerAction {
	return func(pr *printer, doc *Document) printerAction {
		...
		if statement < len(doc.Triples) {
			t = doc.Triples[statement]
		}
...
// printTriple writes one statement, without its line ending.
//
//	triple ::= subject predicate object '.'
//
// The single space between the parts is what §4 requires of canonical
...
// Encode writes triples to w as canonical N-Triples (§4).
//
...
//
// There are no comments, canonical N-Triples having none. A caller wanting to
// keep the comments of a document has a [Document] and wants [Print].
//
// # What canonical form does and does not fix
//
// §4 settles the writing — the spacing, the escaping, the absence of comments
// — so a ground graph encoded twice, here or anywhere, is the same bytes.
//
