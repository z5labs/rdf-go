...
//
// [ntriples.Decode] is drained as well because it is the other entry point a
// document arrives through, and it runs the lowering to data model terms that
// [ntriples.Parse] does not.
//
// The seed corpus is the RDF 1.1 N-Triples suite, whose bad-syntax documents
// are the negative cases, together with the RDF 1.2 suite, every document of
// which this parser either reads as RDF 1.1 or refuses.
func FuzzParse(f *testing.F) {
	seeds, err := fuzzcorpus.Seeds(".nt", "rdf11/rdf-n-triples", "rdf12/rdf-n-triples")
	if err != nil {
	...
	f.Fuzz(func(t *testing.T, src string) {
		for _, err := range ntriples.Decode(strings.NewReader(src)) {
			if err != nil {
				break
			}
		}
		...
		reparsed, err := ntriples.Parse(strings.NewReader(printed))
		if err != nil {
