...
//
// TriG gives a fuzzer more to work with than the line-based syntaxes do:
// graph blocks hold the whole of Turtle, whose collections and blank node
// property lists nest, so a mutation that adds a brace or a bracket adds a
// level of recursion, and the parser has to end rather than descend for as
...
//
// [trig.Decode] is drained as well because it is the other entry point a
// document arrives through, and it runs the prefix and base resolution and the
// lowering to data model terms that [trig.Parse] does not.
//
// The seed corpus is the RDF 1.1 TriG suite, whose bad-syntax documents are
// the negative cases, together with the RDF 1.2 suite, every document of which
// this parser either reads as RDF 1.1 or refuses.
func FuzzParse(f *testing.F) {
	seeds, err := fuzzcorpus.Seeds(".trig", "rdf11/rdf-trig", "rdf12/rdf-trig")
	if err != nil {
	...
	f.Fuzz(func(t *testing.T, src string) {
		...
		reparsed, err := trig.Parse(strings.NewReader(printed))
		if err != nil {
