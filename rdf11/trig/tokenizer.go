// This file began as a copy of rdf11/turtle/tokenizer.go and keeps its
...
// which is where the third copy of them should stop being written by hand:
// Pos, Token, the five error types, tokenizer and its methods, the
// combinators, and every character class from isPNCharsBase down.
//
...
// (https://www.w3.org/TR/trig/#sec-grammar).
...
// TokenType is the kind of a [Token], one per terminal of the TriG grammar
// (https://www.w3.org/TR/trig/#sec-grammar).
type TokenType int
...
const (
	...
	// as "PREFIX" and "BASE" are, so its value is the keyword as written and a
	// printer can put back the spelling the author chose.
	TokenGraph

	// TokenOpenBrace is the "{" that opens a graph block.
	TokenOpenBrace

	// TokenCloseBrace is the "}" that closes one.
	TokenCloseBrace
...
// Tokenize reads the TriG document in r and yields its tokens in order.
//
...
//	}
//
// White space between terminals is dropped, a line ending included: a
// statement ends at its '.' and a graph block at its '}', so nothing
// downstream needs to be told where the lines were. A comment is yielded as
// [TokenComment] rather than dropped, so that everything but indentation
// survives a read and a write.
...
//	WS ::= #x20 | #x9 | #xD | #xA
//
// A line ending is white space here, where N-Quads makes it a terminal. TriG
// statements end at a '.' and graph blocks at a '}', so a document may be laid
// out however the author likes and nothing downstream needs to be told where
// the lines were — only the positions record that.
...
// TriG allows there.
//
// Only "a", "true", "false", "PREFIX", "BASE" and "GRAPH" may stand without a
// colon after them. Anything else that looks like a name is a prefixed name
// missing its colon, which is what this says rather than pointing at whatever
// followed it.
