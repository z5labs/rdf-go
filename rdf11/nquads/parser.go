...
type Document struct {
	...
	// Quads are the document's statements, in the order they were written.
	Quads []*Quad
...
// Quad is one statement of a document.
//
//	statement ::= subject predicate object graphLabel? '.'
//
// The positions the grammar allows each term are expressed in the field types:
// a predicate can only ever be an [*IRIRef], and a [*Literal] is neither a
// [SubjectTerm] nor a [GraphTerm], so it can be neither a subject nor a graph
// label.
type Quad struct {
	...
	//
	// This is the whole of what N-Quads adds to N-Triples.
	Graph GraphTerm
}
...
// no caller assembling a [Quad] by hand can express it.
...
// why a literal stands in neither.
...
func (d *Document) statement(t *Quad) error {
	d.Quads = append(d.Quads, t)
	return nil
}
...
// then either ends the document or hands over to parseStatement.
...
//	statement ::= subject predicate object graphLabel? '.'
//
// Nothing may come between the parts, not even a comment: a comment runs to
// the end of its line, and a statement does not outlive its own line.
